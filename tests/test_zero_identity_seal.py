...
def _resolve_evidence_commit(implementation: str) -> str:
    ...
    raise AssertionError(
        ...
        "second parent of one merge on HEAD's first-parent history"
    )
...
class ZeroIdentitySealTest(unittest.TestCase):
    ...
    def test_evidence_binding_rejects_nested_merge_off_first_parent_history(self) -> None:
        """Reject an evidence merge nested inside the feature side of the main merge."""
        with tempfile.TemporaryDirectory() as td:
            repo = Path(td)
            ...
            run("checkout", "-b", "evidence-feature")
            (repo / "implementation.txt").write_text("implementation\n", encoding="utf-8")
            run("add", "implementation.txt")
            run("commit", "-m", "implementation")
            implementation = git("rev-parse", "HEAD")
            ...
            run("add", evidence_relative)
            run("commit", "-m", "evidence")
            run("checkout", "-b", "integration", base)
            run("merge", "--no-ff", "evidence-feature", "-m", "nested evidence merge")
            (repo / "later.txt").write_text("unsealed feature change\n", encoding="utf-8")
            run("add", "later.txt")
            run("commit", "-m", "later feature change")
            run("checkout", "main")
            run("merge", "--no-ff", "integration", "-m", "human main merge")
