...
def evaluate_use(
    ...
    *,
    ...
    purpose_specs = policy.get('purposes')
    token_decisions = policy.get('token_decisions')
    status_floors = policy.get('status_floors')
    if not all(isinstance(value, dict) for value in (order, purpose_specs, token_decisions, status_floors)):
        raise PurposeUseError('Stage 7 policy structure is invalid')
    ...
    for purpose in normalized:
        ...
        attribution_values = policy.get('attribution_values_requiring_obligations', [])
        if (
            decision == 'ALLOW'
    ...
    overall = _strictest([item['decision'] for item in per_purpose], order)
