...
def collect_errors(root: Path = ROOT) -> list[str]:
    ...
    if not all(isinstance(x,dict) for x in (token_decisions,purpose_specs,status_floors)):
        errors.append('Stage 7 policy mapping structures invalid')
        return errors
    ...
    except (PurposeUseError, ValueError, OSError, json.JSONDecodeError) as exc:
