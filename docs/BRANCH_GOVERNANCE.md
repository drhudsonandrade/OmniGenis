...
- `.github/governance/main-approval-ruleset.json` - owner-only update restriction plus pull-request/review policy, with the approved User bypass actor represented by provider-stable identity rather than a persisted account name.
...
For the current user-owned repository workflow, the formal GitHub approving-review count is zero and `require_last_push_approval` is disabled. This does not authorize automatic merge: the repository owner must still make the final merge action manually after every required status check is green and every review conversation is resolved. The approval ruleset also restricts updates to `main`; only the repository-owner bypass actor can bypass that restriction, and the bypass is limited to pull requests. GitHub documents this mode as requiring the actor to open a pull request while still allowing that actor to choose bypass when merging it, preserving the PR trail without granting ordinary collaborators direct or merge access to `main`. Required status checks remain in the separate protected-main ruleset with no bypass actors. If an independent human reviewer becomes an enforced part of the operating model, the formal approval count can be raised again in a separately reviewed governance change.
