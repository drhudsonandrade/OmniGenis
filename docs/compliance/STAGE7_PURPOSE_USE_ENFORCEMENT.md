...
## Runtime enforcement in report 06

Stage 7 is not only a repository-validation rule. `scripts/build_pharmacogenomic_report.py` evaluates `cpic + REPORT_GENERATION` before it computes the completeness matrix, passport, or report payload. If the decision is not machine-authorized, the CLI exits non-zero before writing any derived report artifact.

Under the current Stage 6 rights record, CPIC `REPORT_GENERATION` resolves to `REVIEW_REQUIRED`; therefore report 06 is intentionally blocked at this boundary until the relevant rights review changes the governed policy. No implicit override or local bypass is introduced.

When a future governed policy legitimately returns an authorized decision, the report compiler records the Stage 7 resource, purposes, decision, and obligations inside the anchored `execution_manifest` so the downstream payload retains the conditions attached to that authorization.
